---
title: "Regression Beyond the Mean"
subtitle: "DSCI 562 Class Meeting 2"
date: ""
output: 
    html_document:
        keep_md: true
...
```{r}
library(tidyverse)
library(Lahman)    # For baseball data example
...
- Why minimizing the sum of squared errors gives us the mean, and why MSE is a measure of goodness. 
- (Perhaps lead-in to quantile regression with median regression)
- Provide an interpretation of a quantile, and how to estimate one from a univariate sample.
    - Compare to mean regression by converting this to an optimization problem, to lead into regression.
- Synthesize an appropriate measure of model goodness for quantile regression model. Understand whether or not MSE is an appropriate measure of goodness for quantile regression. What is the measure for median regression? 
- Explain what quantile regression is, and interpret the corresponding quantile estimates/predictions.
- Explain when quantile regression is useful, and when it's not -- particularly with regards to high quantiles, low quantiles, and median. 
- Explain what Probabilistic forecasting is, and interpret the corresponding probabilistic predictions. Understand that pdf, cdf, and quantile functions are all valid outputs here.
- Explain when probabilistic forecasts are useful, and when they're not. 
- Understand how to obtain probabilistic forecasts from linear regression with a Gaussian error assumption. 
    - Note to myself: this may be non-trivial! Linking concepts such as the mean, linear form, and distribution to the conditional distribution may not have happened.
- Understand how to obtain probabilistic forecasts using local methods (kNN and moving-windows): density only, connecting kernel density estimation from 531 (a univariate estimator).
- Understand what an empirical cdf is, and the logic behind it as an estimator. Understand that an empirical cdf is always a step function.
- Synthesize a kNN/moving-window estimator of cdf using ecdf. 
- Synthesize a probabilistic forecast (of quantile function) using linear quantile regression.

Lab:

- How to fit a linear quantile regression model in R (using both `ggplot2` and the `quantreg` package), and how to interpret it.
- Identify the "crossing quantile" problem.
    - Know how to estimate a quantile in a univariate sample (both in R and the "formula", but not the optimization problem)


Bear in mind that, for the quiz, live coding will not be required. 

# Orientation within the Big-Picture of Supervised Learning

Up until now, we've only seen different ways of using a predictor to give us more information the __mean__ of the response. 

- 561, 562: Data are numeric, so we try to estimate the __mean__.
- 571: Data are categorical, so we try to estimate the __mode__.

The world holds a huge emphasis on the mean and mode, but these are not always what's important. Two alternatives are:
...
There are many ways you can go about modelling and estimating probabilistic forecasts -- in fact, you've already seen some.
